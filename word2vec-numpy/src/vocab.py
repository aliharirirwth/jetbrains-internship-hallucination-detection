"""Vocabulary builder: word2idx, counts, frequencies, and unigram^(3/4) noise table."""
...
class Vocabulary:
    """Vocabulary from raw text with subsampling probabilities and O(1) negative sampling table."""
    ...
    def build(self, tokens: list[str]) -> None:
        """Build vocabulary from token list. Words with count < min_count are excluded."""
    ...
    def subsample_prob(self, word: str, t: float = 1e-5) -> float:
        """Probability of KEEPING the word (Mikolov: P_discard = 1 - sqrt(t / freq(w)))."""
    ...
    def get_noise_table(self, table_size: int = 100_000_000) -> np.ndarray:
        """Build unigram noise table: each word fills slots proportional to freq(w)^(3/4).
        Enables O(1) negative sampling by uniform index into this table."""
